...
class DataPreparationModule:
    ...
    def chunk_documents(self) -> List[Document]:
        ...
        Returns:
            分块后的文档列表
        """
        logger.info(f"正在进行Markdown结构感知分块...")
    ...
    def _markdown_header_split(self) -> List[Document]:
        ...
        for doc in self.documents:
            try:
                ...
                # 如果没能分割成功，说明文档可能没有标题结构
                if len(md_chunks) <= 1:
                    logger.warning(f"文档{doc.metadata.get('dish_name', '未知')}未能按照标题分割，可能没有标题结构")
        ...
        logger.info(f"Markdown结构化分割完成，生成{len(all_chunks)}个结构化块")
        return all_chunks
